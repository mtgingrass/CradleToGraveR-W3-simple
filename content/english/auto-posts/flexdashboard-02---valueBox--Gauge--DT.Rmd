---
title: Practical R for Business - flexdashboard 02   valueBox  Gauge  DT
date: 2019-10-08
categories:
...
<a href="https://www.youtube.com/watch?v=ef8qvUeopN4">
<img src="auto-posts/images/4-LiveArchived-CradleToGraveRLiveStream-Usingpredictfunction.mp4.jpg">
</a>
...
well I go from two and then I have three here where chart a is so it's indented
...
no graphs so if we refresh it we have chart a a2 and a3 going down now we've
...
column okay now we have that figured out let's say that we wanted to make these
...
before we actually put them in our layouts and normally you want to
...
GG plot this is to get you started so let's create some very basic plots that
...
plot and then inside here we want to put our data our data is my data and what I
...
that's it nice and simple command enter on that and WS was not found oh because
...
chart a belongs so down here chart a let's just type in chart a and then I
...
it's going to be the same data my day and this time for our x-axis we'll use x
...
times and here we go it's being rendered refresh and chart a two there it is see
...
aesthetically pleasing so you might have a bunch of different values that you
...
gauge what's a gauge well you'll see let's just let's just put it in there
...
warning so success equals and we're gonna put in a a set of numbers we're
...
again this is a very very introductory not really going to teach you a whole
...
two numbers fact you can do that here just to test it out save you some time
...
they are chunk again is the backticks this little our situation here you have
...
to expert and say what do I want to fill that in with well maybe I want the
...
you can see down here I'm what pag Nate paginated whatever it's called tag nated
...
the one I'm going to pick hopefully you can find it using Google as well and
...
extinctions equal to but and then we have an options list and the
...
bonus you don't probably want to work with empty cars you want to work with
...
this like you don't want to do C Drive backslash your path you can do C Drive
...
it's right here copy this command C and then let's go back over to here and we
...
height dot in dot so hide in anxious basically so let's go ahead and command
...
whole script or actually highlight the whole thing click on run get to run the
...
whatsoever so I hope that you can modify this and create your own dashboardsEnd of file
